...
function createBot(userId, phoneNumber, res) {
    ...
    client.on('message', async (msg) => {
        ...
        if (text === '.menu') {
            const menu = `*${BOT_NAME}*\n\n` +
                `📌 *Commands:*\n\n` +
                `1️⃣ *.follow* → Channel follow\n\n` +
                `2️⃣ *.postid <link>* → Post ID ගන්න\n\n` +
                `3️⃣ *.create <link>* → Post එකට react\n\n` +
                `4️⃣ *.stats* → Statistics බලන්න\n\n` +
                `5️⃣ *.dontreactandfollow <password>* → නවත්තන්න\n\n` +
                `6️⃣ *.menu* → මේ menu එක\n\n` +
                `🔗 ${CHANNEL_LINK}`;
        ...
        if (text === '.follow') {
            ...
            try {
                ...
                await msg.reply(
                    ...
                    `📊 Follow Statistics:\n` +
                    `👥 මුළු users: ${db.totalUsers.length}\n` +
                    `✅ Follow කරපු: ${db.stats.totalFollows}\n` +
                    `❌ ඉතිරි: ${db.totalUsers.length - db.stats.totalFollows}\n\n` +
                    `🔗 ${CHANNEL_LINK}`
                );
            } catch (e) {
        ...
        if (text.startsWith('.postid')) {
            ...
            const match = parts[1].match(/channel\/([^\/]+)\/(\d+)/);
            if (match) {
                await msg.reply(
                    `✅ Post ID එක ගත්තා!\n\n` +
                    `Channel: ${match[1]}\n` +
                    `Post ID: ${match[2]}\n` +
                    `Full ID: ${match[1]}@newsletter_${match[2]}`
                );
            } else {
        ...
        if (text.startsWith('.create')) {
            ...
            try {
                ...
                await msg.reply(
                    ...
                    `📊 React Statistics:\n` +
                    `👥 මුළු users: ${db.totalUsers.length}\n` +
                    `✅ React කරපු: ${db.stats.totalReacts}\n` +
                    `❌ ඉතිරි: ${db.totalUsers.length - db.stats.totalReacts}`
                );
            } catch (e) {
        ...
        if (text === '.stats') {
            await msg.reply(
                ...
                `📌 Follow:\n` +
                `✅ කරපු: ${db.stats.totalFollows}\n` +
                `❌ ඉතිරි: ${db.totalUsers.length - db.stats.totalFollows}\n\n` +
                `❤️ React:\n` +
                `✅ කරපු: ${db.stats.totalReacts}\n` +
                `❌ ඉතිරි: ${db.totalUsers.length - db.stats.totalReacts}`
            );
            return;
        }
