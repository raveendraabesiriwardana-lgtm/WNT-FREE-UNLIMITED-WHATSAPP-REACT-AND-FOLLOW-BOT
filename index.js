...
function findChromePath() {
    if (process.env.PUPPETEER_EXECUTABLE_PATH && fs.existsSync(process.env.PUPPETEER_EXECUTABLE_PATH)) {
        console.log('Chrome found via PUPPETEER_EXECUTABLE_PATH');
        return process.env.PUPPETEER_EXECUTABLE_PATH;
    }

    // Project folder එකේ .cache/puppeteer
    const localCacheDir = path.join(__dirname, '.cache', 'puppeteer', 'chrome');
    if (fs.existsSync(localCacheDir)) {
        try {
            const versions = fs.readdirSync(localCacheDir);
            for (const ver of versions) {
                const chromePath = path.join(localCacheDir, ver, 'chrome-linux64', 'chrome');
            ...
            console.log('Local cache scan error:', e.message);
        }
    }

    // /app/.cache/puppeteer
    const appCacheDir = '/app/.cache/puppeteer/chrome';
    if (fs.existsSync(appCacheDir)) {
        try {
            const versions = fs.readdirSync(appCacheDir);
            for (const ver of versions) {
                const chromePath = path.join(appCacheDir, ver, 'chrome-linux64', 'chrome');
                if (fs.existsSync(chromePath)) {
                    console.log('Chrome found at:', chromePath);
                    return chromePath;
                }
